...
#define RSTDIO_QUEUE_LENGTH        256 /* items in queue, that's my buffer size */
#define RSTDIO_QUEUE_ITEM_SIZE     1 /* each item is a single character */

#define RSTDIO_QUEUE_TIMEOUT_MS   1500 /* timeout for stdio queues */
...
static uint8_t AddToQueue(xQueueHandle queue, const unsigned char *data, size_t dataSize) {
  while(dataSize!=0) {
    if (FRTOS1_xQueueSendToBack(queue, data, RSTDIO_QUEUE_TIMEOUT_MS/portTICK_PERIOD_MS)!=pdPASS) {
      return ERR_FAULT;
    }
    data++;
    dataSize--;
